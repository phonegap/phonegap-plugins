...
//    [repeatDict setObject:[NSNumber numberWithInt:0] forKey:@""];
	...
	notif.applicationIconBadgeNumber = badge; // set to incoming value
	...
	// added foreground and background notifications
	NSLog(@"Notification Set: %@ (ID: %@, Repeat: %@, Badge: %i, sound: %@,bg callback: %@,fg callback: %@)", date, notificationId, repeat, badge, sound,bg,fg);
	[notif release];
}
...
// prepare for cancelling individual notifications
