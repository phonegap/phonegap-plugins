...
/*import new version cordova lib
 * 
 */
...
public class LocalNotification extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray args,
		...
		if (action.equalsIgnoreCase("add")) {
			...
			callbackContext.sendPluginResult(this.add(daily, title, subTitle, ticker, alarmId,
					alarmOptions.getCal()));
			return true;
		} else if (action.equalsIgnoreCase("cancel")) {
			unpersistAlarm(alarmId);
			callbackContext.sendPluginResult(this.cancelNotification(alarmId));
			return true;
		} else if (action.equalsIgnoreCase("cancelall")) {
			unpersistAlarmAll();
			callbackContext.sendPluginResult(this.cancelAllNotifications());
			return true;
		}
	...
	public PluginResult cancelAllNotifications() {
		 ...
		 * by one. 11-11 15:03:26.301: I/Deprecation Notice(7803): Replace
		 * ctx.getSharedPreferences() with
		 * cordova.getActivity().getSharedPreferences()
		 */
		final SharedPreferences alarmSettings = this.cordova.getActivity()
